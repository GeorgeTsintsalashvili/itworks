...
use App\Http\Requests\UpdateNotebookChargerRequest;
use App\Http\Requests\StoreNotebookChargerRequest;

use App\Rules\NaturalNumber;
use App\Rules\BinaryValue;
use App\Rules\PositiveIntegerOrZero;

use \App\Models\ControlPanel\NotebookCharger;
...
class NotebookChargerController extends Controller
{
    public function index(Request $request) // Display a listing of the resource
    {
      ...
      $queryBuilder = \DB::table('notebook_chargers');
      ...
      $priceRanges = \DB::table('price_configurations') -> select(['notebookChargerMinPrice', 'notebookChargerMaxPrice']) -> first();
      $productMinPrice = $priceRanges -> notebookChargerMinPrice;
      $productMaxPrice = $priceRanges -> notebookChargerMaxPrice;
      ...
      return \View::make('contents.controlPanel.notebookChargers.index') -> with([
    ...
    public function store(StoreNotebookChargerRequest $request) // Store a newly created resource in storage
    {
      ...
      if($request -> file('mainImage') -> isValid())
      {
        ...
        $fileName = MainImageUploadable::uploadMainImage(NotebookCharger::class, $file);
        ...
        if($fileName)
        {
          ...
          if($request -> has('images'))
          {
            ...
            foreach($carouselImages as $carouselImage)
            {
              ...
              {
                CarouselImageUploadable::uploadImage(NotebookCharger::class, $object -> id, $carouselImage);
              }
            }
          }
    ...
    public function edit($id) // Show the form for editing the specified resource
    {
      try{
        ...
        $product = NotebookCharger::findOrFail($id);

        $warranties = \DB::table('warranties') -> get();
        $images = \DB::table('notebook_chargers_images') -> where('notebookChargerId', $id) -> get();

        $manufacturers = \DB::table('notebooks_manufacturers') -> get();

        return \View::make('contents.controlPanel.notebookChargers.edit') -> with([
    ...
    public function update(UpdateNotebookChargerRequest $request) // Update the specified resource in storage
    {
      $data = $request -> validated();
      ...
      try{
            ...
            $record = NotebookCharger::findOrFail($recordId);
    ...
    public function destroy($id) // Remove the specified resource from storage
    {
      try{
        ...
        RecordDeletable::deleteRecord(NotebookCharger::class, $id);
    ...
    public function updateBaseData(BaseDataUpdateRequest $request)
    {
      ...
      try{
        ...
        BaseDataUpdatable::updateBaseData(NotebookCharger::class, $data);
      ...
      if($request -> hasFile('image')) // if file is moved into temporary location by web server
      {
        ...
        try{
              ...
              if(!$validator -> fails())
              {
                ...
                $imagesSources = MainImageUpdatable::updateImage(NotebookCharger::class, $recordId, $file);
      ...
      if($request -> hasFile('image')) // if file is moved into temporary location by web server
      {
        ...
        try{
              ...
              if(!$validator -> fails())
              {
                ...
                $file = $request -> file('image');
                $className = NotebookCharger::class;
                $recordId = $request -> input('record-id');
    ...
    public function destroyImage($id)
    {
      ...
      $classBaseName = class_basename(NotebookCharger::class);
    ...
    public function parameters(Request $request)
    {
      ...
      $ncmTypesToView = 6;
      $ncmTypesNum = \DB::table('notebooks_manufacturers') -> count();
      $ncmTypesPage = $validator -> fails() ? 1 : (int) $parameters[$ncmTypesPageKey];
      $ncmTypesToSkip = ($ncmTypesPage - 1) * $ncmTypesToView;

      return \View::make('contents.controlPanel.notebookChargers.parameters') -> with([
         'manufacturers' => \DB::table('notebooks_manufacturers') -> skip($ncmTypesToSkip) -> take($ncmTypesToView) -> orderBy('id', 'desc') -> get(),
         'ncmPaginator' => \Paginator::build($ncmTypesNum, 2, $ncmTypesToView, $ncmTypesPage, 2, 2),
         'ncmPageKey' => $ncmTypesPageKey
      ]);
    }

    public function storeNotebookChargerManufacturer(Request $request)
    {
      $data['success'] = false;
      ...
      if(!$validator -> fails())
      {
         ...
         \DB::table('notebooks_manufacturers') -> insert($dataToStore);
      }

      return $data;
    }

    public function updateNotebookChargerManufacturer(Request $request)
    {
      $data['updated'] = false;
      ...
      if(!$validator -> fails())
      {
         ...
         \DB::table('notebooks_manufacturers') -> where('id', $parameters['record-id']) -> update(['manufacturerTitle' => $parameters['manufacturer-title']]);
      }

      return $data;
    }

    public function destroyNotebookChargerManufacturer($id)
    {
      $data['deleted'] = false;

      $numOfNotebookChargers = \DB::table('notebook_chargers') -> where('notebookManufacturerId', $id) -> count();

      if($numOfNotebookChargers == 0)
      {
        $data['deleted'] = true;

        \DB::table('notebooks_manufacturers') -> where('id', $id) -> delete();
      }
