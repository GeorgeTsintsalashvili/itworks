...
use \App\Models\ControlPanel\NetworkDevice;
...
class NetworkDeviceController extends Controller
{
    public function index(Request $request) // Display a listing of the resource
    {
      ...
      $queryBuilder = \DB::table('network_devices');
      ...
      $priceRanges = \DB::table('price_configurations') -> select(['networkDeviceMinPrice', 'networkDeviceMaxPrice']) -> first();
      $productMinPrice = $priceRanges -> networkDeviceMinPrice;
      $productMaxPrice = $priceRanges -> networkDeviceMaxPrice;

      $types = \DB::table('network_devices_types') -> get();
      ...
      return \View::make('contents.controlPanel.networkDevices.index') -> with([
    ...
    public function store(StoreNetworkDeviceRequest $request) // Store a newly created resource in storage
    {
      ...
      if($request -> file('mainImage') -> isValid())
      {
        ...
        $fileName = MainImageUploadable::uploadMainImage(NetworkDevice::class, $file);
        ...
        if($fileName)
        {
          ...
          if($request -> has('images'))
          {
            ...
            foreach($carouselImages as $carouselImage)
            {
              ...
              {
                CarouselImageUploadable::uploadImage(NetworkDevice::class, $object -> id, $carouselImage);
              }
            }
          }
    ...
    public function edit($id) // Show the form for editing the specified resource
    {
      try{
        ...
        $product = NetworkDevice::findOrFail($id);

        $warranties = \DB::table('warranties') -> get();
        $images = \DB::table('network_devices_images') -> where('networkDeviceId', $id) -> get();

        $types = \DB::table('network_devices_types') -> get();

        return \View::make('contents.controlPanel.networkDevices.edit') -> with([
    ...
    public function update(UpdateNetworkDeviceRequest $request) // Update the specified resource in storage
    {
      $data = $request -> validated();
      ...
      try{
            ...
            $record = NetworkDevice::findOrFail($recordId);
    ...
    public function destroy($id) // Remove the specified resource from storage
    {
      try{
        ...
        RecordDeletable::deleteRecord(NetworkDevice::class, $id);
    ...
    public function updateBaseData(BaseDataUpdateRequest $request)
    {
      ...
      try{
        ...
        BaseDataUpdatable::updateBaseData(NetworkDevice::class, $data);
      ...
      if($request -> hasFile('image')) // if file is moved into temporary location by web server
      {
        ...
        try{
              ...
              if(!$validator -> fails())
              {
                ...
                $imagesSources = MainImageUpdatable::updateImage(NetworkDevice::class, $recordId, $file);
    ...
    public function destroyImage($id)
    {
      ...
      $classBaseName = class_basename(NetworkDevice::class);
    ...
    public function parameters(Request $request)
    {
      ...
      $ndTypesToView = 6;
      $ndTypesNum = \DB::table('network_devices_types') -> count();
      $ndTypesPage = $validator -> fails() ? 1 : (int) $parameters[$ndTypesPageKey];
      $ndTypesToSkip = ($ndTypesPage - 1) * $ndTypesToView;

      return \View::make('contents.controlPanel.networkDevices.parameters') -> with([
         'ndTypes' => \DB::table('network_devices_types') -> skip($ndTypesToSkip) -> take($ndTypesToView) -> orderBy('id', 'desc') -> get(),
         'ndTypesPaginator' => \Paginator::build($ndTypesNum, 2, $ndTypesToView, $ndTypesPage, 2, 2),
         'ndTypesPageKey' => $ndTypesPageKey
      ]);
    }

    public function storeNetworkDeviceType(Request $request)
    {
      $data['success'] = false;

      $parameters = $request -> all(); // user input

      $validator = \Validator::make($parameters, [ 'network-device-type-title' => 'required|string|min:1|max:100',
                                                   'network-device-type-icon' => 'string|nullable|max:100' ]);

      if(!$validator -> fails())
      {
         $data['success'] = true;

         $dataToStore = ['typeTitle' => $parameters['network-device-type-title']];

         if($request -> filled('network-device-type-icon'))
         {
           $dataToStore['icon'] = $parameters['network-device-type-icon'];
         }

         \DB::table('network_devices_types') -> insert($dataToStore);
      }

      return $data;
    }

    public function updateNetworkDeviceType(Request $request)
    {
      $data['updated'] = false;
      ...
      if(!$validator -> fails())
      {
         ...
         $valuesToPass = [ 'typeTitle' => $parameters['network-device-type-title'] ];

         $valuesToPass['icon'] = $request -> filled('network-device-type-icon') ? $parameters['network-device-type-icon'] : 'fas fa-square';

         \DB::table('network_devices_types') -> where('id', $parameters['record-id']) -> update($valuesToPass);
      }

      return $data;
    }

    public function destroyNetworkDeviceType($id)
    {
      $data['deleted'] = false;

      $numOfNetworkDevices = \DB::table('network_devices') -> where('typeId', $id) -> count();

      if($numOfNetworkDevices == 0)
      {
        $data['deleted'] = true;

        \DB::table('network_devices_types') -> where('id', $id) -> delete();
      }
