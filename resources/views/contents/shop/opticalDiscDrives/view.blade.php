...
@if($contentData['opticalDiscDriveExists'])
 ...
 <div class="container">
  <div class="row">
   <div class="col-md-12">
    ...
    <a class="breadcrumb-part breadcrumb-link" href="/opticalDiscDrives">
     <span class="font-5">ოპტიკური დისკმძრავები</span>
    </a>
    <span class="breadcrumb-part font-5">{{ $contentData['opticalDiscDrive'] -> title }}</span>
   </div>
  </div>
 </div>
</div>
       ...
       <div class="image-block-wrapper position-relative">
        ...
        <div id="image-block" class="clearfix">
         <span id="view-full-size">
          ...
          <img id="big-image" src="/images/opticalDiscDrives/main/original/{{ $contentData['opticalDiscDrive'] -> mainImage }}" width="800" height="850" />
       ...
       <div id="product-images-slider" class="splide">
       	<div class="splide__track">
            ...
            <li class="splide__slide">
             <a href="/images/opticalDiscDrives/main/original/{{ $contentData['opticalDiscDrive'] -> mainImage }}" data-fancybox="other-views" class="fancybox shown">
              <img class="img-fluid" src="/images/opticalDiscDrives/main/preview/{{ $contentData['opticalDiscDrive'] -> mainImage }}" />
             </a>
            </li>
            @if($contentData['imagesExist'])
            @foreach($contentData['images'] as $value)
            <li class="splide__slide">
             <a href="/images/opticalDiscDrives/slides/original/{{ $value -> image }}" data-fancybox="other-views" class="fancybox">
              <img class="img-fluid" src="/images/opticalDiscDrives/slides/preview/{{ $value -> image }}"/>
             </a>
            </li>
            @endforeach
            @endif
      ...
      <div class="col-md-7 col-sm-12 col-xs-12 info">
       <div class="product-details-title-wrapper">
        <div class="product-details-title">
         <h1 class="font-4">{{ $contentData['opticalDiscDrive'] -> title }}</h1>
        </div>
       </div>
       ...
       <div class="prices-container">
        <p class="current-price-wrapper no-margin">
         <span id="current-price">₾ {{ $contentData['opticalDiscDrive'] -> newPrice }} </span>
        </p>
        @if($contentData['opticalDiscDrive'] -> discount != 0)
        <p class="old-price-wrapper no-margin">
         <span id="old-price"> ₾ {{ $contentData['opticalDiscDrive'] -> price }} </span>
        </p>
        @endif
       </div>
       <!--- price details end --->
       ...
       <p id="product-warranty">
        ...
        <span class="font-8">მოქმედებს {{ $contentData['opticalDiscDrive'] -> warrantyDuration }} {{ $contentData['warrantyTitle']}} გარანტია საუკეთესო პირობებით</span>
       </p>
       ...
       <p id="product-code">
        ...
        <span>{{ $contentData['opticalDiscDrive'] -> categoryId }}-{{ $contentData['opticalDiscDrive'] -> id }}</span>
       </p>

       @if($contentData['opticalDiscDrive'] -> quantity != 0)
       <p id="product-quantity">
        <b class="font-6">საერთო რაოდენობა </b>
        <span>{{ $contentData['opticalDiscDrive'] -> quantity }}</span>
       </p>
       @endif
       ...
       <div class="social-networks-wrapper">
        <div class="fb-share-button" data-href="/opticalDiscDrives/{{ $contentData['opticalDiscDrive'] -> id }}" data-size="large" data-layout="button_count" style="position: absolute;"></div>
       </div>
       ...
       <form id="add-to-cart-form" action="/shop/cart/add" method="post" data-allowed-quantity="{{ $contentData['opticalDiscDrive'] -> quantity }}">
           ...
           <div class="quantity-product-option">
            <div id="quantity-wanted-wrapper">
             <div class="quantity-value-option">
              ...
              <input type="hidden" name="product-id" value="{{ $contentData['opticalDiscDrive'] -> id }}" />
              <input type="hidden" name="category-id" value="{{ $contentData['opticalDiscDrive'] -> categoryId }}" />
    ...
    <div class="col-lg-12">
      <div class="tabs-block">
       ...
       <div class="tab-content" id="tab-content">
        <div class="tab-pane fade show active" id="description" role="tabpanel" aria-labelledby="description-tab">{!! $contentData['opticalDiscDrive'] -> description !!}</div>
...
@if($contentData['recommendedOpticalDiscDrivesExist'])
          ...
          <div class="itw-top-product-inner-category">
           ...
           <div class="product-wrapper splide__track">
            ...
            <ul class="product-list itw-product-top splide__list">
             @foreach($contentData['recommendedOpticalDiscDrives'] as $key => $value)
              ...
              <div class="product-container">
               <div class="itw-display-product-info">
                <div class="left-block">
                 <div class="product-image-container">
                  <a href="/opticalDiscDrives/{{ $value -> id }}">
                   <img class="replace-2x img-fluid" src="/images/opticalDiscDrives/main/original/{{ $value -> mainImage }}" />
                  </a>
                 </div>
                </div>
                ...
                <div class="right-block">
                 <h5>
                  <a class="product-name font-7" href="/opticalDiscDrives/{{ $value -> id }}">
               ...
               <div class="product-more-options">
                <div class="read-more-container">
                 <a class="button read-more-button btn btn-secondary" href="/opticalDiscDrives/{{ $value -> id }}">
