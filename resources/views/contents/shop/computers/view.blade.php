...
@section('facebookShareImage', '/images/facebookShareImages/sistemuri.png')
 ...
 <div class="container">
  <div class="row">
   <div class="col-md-12">
    ...
    <a class="breadcrumb-part breadcrumb-link" href="/computers">
     <span class="font-5">სისტემური ბლოკები</span>
    </a>
    <span class="breadcrumb-part font-5">{{ $contentData['computer'] -> title }}</span>
   </div>
  </div>
 </div>
</div>
       ...
       <div class="image-block-wrapper position-relative">
        ...
        <div id="image-block" class="clearfix">
         ...
         <span class="product-flag new">
          <span class="font-8"> შემოთავაზება</span>
         </span>
         @endif
         <span id="view-full-size">
          <span class="border-product"></span>
          <img id="big-image" src="/images/computers/main/original/{{ $contentData['computer'] -> mainImage }}" width="800" height="850" />
       ...
       <div id="product-images-slider" class="splide">
       	<div class="splide__track">
            ...
            <li class="splide__slide">
             <a href="/images/computers/main/original/{{ $contentData['computer'] -> mainImage }}" data-fancybox="other-views" class="fancybox shown">
              <img class="img-fluid" src="/images/computers/main/preview/{{ $contentData['computer'] -> mainImage }}" />
             </a>
            </li>
            @if($contentData['imagesExist'])
            @foreach($contentData['images'] as $value)
            <li class="splide__slide">
             <a href="/images/computers/slides/original/{{ $value -> image }}" data-fancybox="other-views" class="fancybox">
              <img class="img-fluid" src="/images/computers/slides/preview/{{ $value -> image }}"/>
             </a>
            </li>
            @endforeach
            @endif
      ...
      <div class="col-md-7 col-sm-12 col-xs-12 info">
       <div class="product-details-title-wrapper">
        <div class="product-details-title">
         <h1 class="font-4">{{ $contentData['computer'] -> title }}</h1>
        </div>
       </div>
       ...
       <div class="prices-container">
        <p class="current-price-wrapper no-margin">
         <span id="current-price">₾ {{ $contentData['computer'] -> newPrice }} </span>
        </p>
        @if($contentData['computer'] -> discount != 0)
        <p class="old-price-wrapper no-margin">
         <span id="old-price"> ₾ {{ $contentData['computer'] -> price }} </span>
        </p>
        @endif
       </div>
       <!--- price details end --->
       ...
       <p id="product-warranty">
        ...
        <span class="font-8">მოქმედებს {{ $contentData['computer'] -> warrantyDuration }} {{ $contentData['warrantyTitle'] }} გარანტია საუკეთესო პირობებით</span>
       </p>
       ...
       <p id="stock-status">
        <a class="stock-status-title" style="background: {{ $contentData['computer'] -> statusColor }}">
         <span> {{ !$contentData['computer'] -> check ? 'სისტემა იწყობა შეკვეთიდან 24 საათში' : 'მარაგის გადასამოწმებლად დაგვიკავშირდით'}}</span>
        </a>
       </p>
       ...
       <p id="product-code">
        ...
        <span>{{ $contentData['computer'] -> categoryId }}-{{ $contentData['computer'] -> id }}</span>
       </p>

       @if($contentData['computer'] -> quantity != 0)
       <p id="product-quantity">
        <b class="font-6">აწყობის მაქსიმალური რაოდენობა </b>
        <span>{{ $contentData['computer'] -> quantity }} </span>
       </p>
       @endif
       ...
       <div class="social-networks-wrapper">
        <div class="fb-share-button" data-href="/computers/{{ $contentData['computer'] -> id }}" data-size="large" data-layout="button_count" style="position: absolute;"></div>
       </div>

       @if($contentData['enableAddToCartButton'])

       <form id="add-to-cart-form" action="/shop/cart/add" method="post" data-allowed-quantity="{{ $contentData['computer'] -> quantity }}">
           ...
           <div class="quantity-product-option">
            <div id="quantity-wanted-wrapper">
             <div class="quantity-value-option">
              ...
              <input type="hidden" name="product-id" value="{{ $contentData['computer'] -> id }}" />
              <input type="hidden" name="category-id" value="{{ $contentData['computer'] -> categoryId }}" />
    ...
    <div class="col-lg-12">
      <div class="tabs-block">
       ...
       <div class="tab-content" id="tab-content">
        <div class="tab-pane fade show active" id="description" role="tabpanel" aria-labelledby="description-tab">{!! $contentData['computer'] -> description !!}</div>
