...
@section('facebookShareImage', '/images/facebookShareImages/aqsesuarebi.png')

@section('content')

@if($contentData['accessoryExists'])
 ...
 <div class="container">
  <div class="row">
   <div class="col-md-12">
    ...
    <a class="breadcrumb-part breadcrumb-link" href="/accessories">
     <span class="font-5">აქსესუარები</span>
    </a>
    <span class="breadcrumb-part font-5"> {{ $contentData['accessory'] -> title }} </span>
   </div>
  </div>
 </div>
</div>
       ...
       <div class="image-block-wrapper position-relative">
        ...
        <div id="image-block" class="clearfix">
         <span id="view-full-size">
          ...
          <img id="big-image" src="/images/accessories/main/original/{{ $contentData['accessory'] -> mainImage }}" width="800" height="850" />
       ...
       <div id="product-images-slider" class="splide">
       	<div class="splide__track">
            ...
            <li class="splide__slide">
             <a href="/images/accessories/main/original/{{ $contentData['accessory'] -> mainImage }}" data-fancybox="other-views" class="fancybox shown">
              <img class="img-fluid" src="/images/accessories/main/preview/{{ $contentData['accessory'] -> mainImage }}" />
             </a>
            </li>
            @if($contentData['imagesExist'])
            @foreach($contentData['images'] as $value)
            <li class="splide__slide">
             <a href="/images/accessories/slides/original/{{ $value -> image }}" data-fancybox="other-views" class="fancybox">
              <img class="img-fluid" src="/images/accessories/slides/preview/{{ $value -> image }}"/>
             </a>
            </li>
            @endforeach
            @endif
      ...
      <div class="col-md-7 col-sm-12 col-xs-12 info">
       <div class="product-details-title-wrapper">
        <div class="product-details-title">
         <h1 class="font-4">{{ $contentData['accessory'] -> title }}</h1>
        </div>
       </div>
       ...
       <div class="prices-container">
        <p class="current-price-wrapper no-margin">
         <span id="current-price">₾ {{ $contentData['accessory'] -> newPrice }} </span>
        </p>
        @if($contentData['accessory'] -> discount != 0)
        <p class="old-price-wrapper no-margin">
         <span id="old-price"> ₾ {{ $contentData['accessory'] -> price }} </span>
        </p>
        @endif
       </div>
       <!--- price details end --->
       ...
       <p id="product-warranty">
        ...
        <span class="font-8">მოქმედებს {{ $contentData['accessory'] -> warrantyDuration }} {{ $contentData['warrantyTitle'] }} გარანტია საუკეთესო პირობებით</span>
       </p>
       ...
       <p id="product-code">
        ...
        <span> {{ $contentData['accessory'] -> categoryId }}-{{ $contentData['accessory'] -> id }} </span>
       </p>

       @if($contentData['accessory'] -> quantity != 0)
       <p id="product-quantity">
        <b class="font-6">საერთო რაოდენობა </b>
        <span>{{ $contentData['accessory'] -> quantity }}</span>
       </p>
       @endif
       ...
       <div class="social-networks-wrapper">
        <div class="fb-share-button" data-href="/accessories/{{ $contentData['accessory'] -> id }}" data-size="large" data-layout="button_count" style="position: absolute;"></div>
       </div>
       ...
       <form id="add-to-cart-form" action="/shop/cart/add" method="post" data-allowed-quantity="{{ $contentData['accessory'] -> quantity }}">
           ...
           <div class="quantity-product-option">
            <div id="quantity-wanted-wrapper">
             <div class="quantity-value-option">
              ...
              <input type="hidden" name="product-id" value="{{ $contentData['accessory'] -> id }}" />
              <input type="hidden" name="category-id" value="{{ $contentData['accessory'] -> categoryId }}" />
    ...
    <div class="col-lg-12">
      <div class="tabs-block">
       ...
       <div class="tab-content" id="tab-content">
        <div class="tab-pane fade show active" id="description" role="tabpanel" aria-labelledby="description-tab">{!! $contentData['accessory'] -> description !!}</div>
...
@if($contentData['recommendedAccessoriesExist'])
          ...
          <div class="itw-top-product-inner-category">
           ...
           <div class="product-wrapper splide__track">
            ...
            <ul class="product-list itw-product-top splide__list">
             @foreach($contentData['recommendedAccessories'] as $value)
              ...
              <div class="product-container">
               <div class="itw-display-product-info">
                ...
                <div class="right-block">
                 <h5>
                  <a class="product-name font-7" href="/accessories/{{ $value -> id }}">
