 ...
 <div class="container">
  <div class="row">
   <div class="col-md-12">
    ...
    <a class="breadcrumb-part breadcrumb-link" href="/computerCases">
     <span class="font-5">კეისები</span>
    </a>
    <span class="breadcrumb-part font-5">{{ $contentData['case'] -> title }}</span>
   </div>
  </div>
 </div>
</div>
       ...
       <div class="image-block-wrapper position-relative">
        ...
        <div id="image-block" class="clearfix">
         <span id="view-full-size">
          ...
          <img id="big-image" src="/images/computerCases/main/original/{{ $contentData['case'] -> mainImage }}" width="800" height="850" />
       ...
       <div id="product-images-slider" class="splide">
       	<div class="splide__track">
            ...
            <li class="splide__slide">
             <a href="/images/computerCases/main/original/{{ $contentData['case'] -> mainImage }}" data-fancybox="other-views" class="fancybox shown">
              <img class="img-fluid" src="/images/computerCases/main/preview/{{ $contentData['case'] -> mainImage }}" />
             </a>
            </li>
            @if($contentData['imagesExist'])
            @foreach($contentData['images'] as $value)
            <li class="splide__slide">
             <a href="/images/computerCases/slides/original/{{ $value -> image }}" data-fancybox="other-views" class="fancybox">
              <img class="img-fluid" src="/images/computerCases/slides/preview/{{ $value -> image }}"/>
             </a>
            </li>
            @endforeach
            @endif
      ...
      <div class="col-md-7 col-sm-12 col-xs-12 info">
       <div class="product-details-title-wrapper">
        <div class="product-details-title">
         <h1 class="font-4">{{ $contentData['case'] -> title }}</h1>
        </div>
       </div>
       ...
       <div class="prices-container">
        <p class="current-price-wrapper no-margin">
         <span id="current-price">₾ {{ $contentData['case'] -> newPrice }} </span>
        </p>
        @if($contentData['case'] -> discount != 0)
        <p class="old-price-wrapper no-margin">
         <span id="old-price"> ₾ {{ $contentData['case'] -> price }} </span>
        </p>
        @endif
       </div>
       <!--- price details end --->
       ...
       <p id="product-warranty">
        ...
        <span class="font-8">მოქმედებს {{ $contentData['case'] -> warrantyDuration }} {{ $contentData['warrantyTitle'] }} გარანტია საუკეთესო პირობებით</span>
       </p>
       ...
       <p id="product-code">
        ...
        <span>{{ $contentData['case'] -> categoryId }}-{{ $contentData['case'] -> id }}</span>
       </p>

       @if($contentData['case'] -> quantity != 0)
       <p id="product-quantity">
        <b class="font-6">საერთო რაოდენობა </b>
        <span>{{ $contentData['case'] -> quantity }} </span>
       </p>
       @endif
       ...
       <div class="social-networks-wrapper">
        <div class="fb-share-button" data-href="/computerCases/{{ $contentData['case'] -> id }}" data-size="large" data-layout="button_count" style="position: absolute;"></div>
       </div>
       ...
       <form id="add-to-cart-form" action="/shop/cart/add" method="post" data-allowed-quantity="{{ $contentData['case'] -> quantity }}">
           ...
           <div class="quantity-product-option">
            <div id="quantity-wanted-wrapper">
             <div class="quantity-value-option">
              ...
              <input type="hidden" name="product-id" value="{{ $contentData['case'] -> id }}" />
              <input type="hidden" name="category-id" value="{{ $contentData['case'] -> categoryId }}" />
    ...
    <div class="col-lg-12">
      <div class="tabs-block">
       ...
       <div class="tab-content" id="tab-content">
        <div class="tab-pane fade show active" id="description" role="tabpanel" aria-labelledby="description-tab">{!! $contentData['case'] -> description !!}</div>
...
@if($contentData['recommendedCasesExist'])
          ...
          <div class="itw-top-product-inner-category">
           ...
           <div class="product-wrapper splide__track">
            ...
            <ul class="product-list itw-product-top splide__list">
             @foreach($contentData['recommendedCases'] as $value)
              ...
              <div class="product-container">
               <div class="itw-display-product-info">
                <div class="left-block">
                 <div class="product-image-container">
                  <a href="/computerCases/{{ $value -> caseId }}">
                   <img class="replace-2x img-fluid" src="/images/computerCases/main/original/{{ $value -> mainImage }}" />
                  </a>
                 </div>
                </div>
                ...
                <div class="right-block">
                 <h5>
                  <a class="product-name font-7" href="/computerCases/{{ $value -> caseId }}">
               ...
               <div class="product-more-options">
                <div class="read-more-container">
                 <a class="button read-more-button btn btn-secondary" href="/computerCases/{{ $value -> caseId }}">
                ...
                <div class="short-description">
                 <div>
                  ...
                  <span class="font-5">{{ $value -> formFactorTitle }}</span>
                 </div>
                </div>
               </div>
              </div>
