	...
	<div class="col-xs-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="font-6 p-abs">დედაპლატები</h4>
				<div class="options">
			...
			<div class="panel-body">
				...
				<div class="tab-content">
				 ...
				 <div class="tab-pane active" id="products-list">
					 ...
					 <table class="table table-hover mb1" data-update-route="{{ route('mbUpdateBase') }}">
	 					<thead>
	 						<tr class="font-6">
	 							<th>სურათი</th>
	 							<th>სათაური</th>
	           ...
	           @foreach($items as $value)
	 						...
	 						<tr data-record-id="{{ $value -> id }}">
	 							...
	 							<td class="preview-image">
	 								<img src="/images/motherboards/main/preview/{{ $value -> mainImage }}" class="invoice-image" data-uuid="f9-{{ $value -> id }}">
	 							</td>
								...
								<td class="text-center">
									<a class="add-warranty mr20" href="f9-{{ $value -> id }}" data-system-part="0" data-warranty-page="parts" data-warranty="{{ $value -> warranty }}" style="line-height: 1">
									 <i style="font-size: 22px" class="fa fa-wrench"></i>
									</a>

								 <a class="add-warranty" href="f9-{{ $value -> id }}" data-system-part="1" data-warranty-page="computer" data-warranty="{{ $value -> warranty }}" style="line-height: 1">
								...
								<td class="text-center">
	 							...
	 							<td class="text-center">
	 							 <a class="view-page" href="{{ route('mbEdit', ['id' => $value -> id]) }}">
	 								<i style="font-size: 18px" class="fas fa-edit" aria-hidden="true"></i>
	 							 </a>
	 							</td>

	 							<td class="text-center">
	 								<a class="delete-record" href="{{ route('mbDestroy', ['id' => $value -> id]) }}">
	         ...
	         <div data-page-key="{{ $paginationKey }}" data-current-page="{{ $paginator -> currentPage }}" class="clearfix pagination-column col-sm-12">
	           ...
	           <div class="col-sm-4">
	             <div class="input-group">
	               ...
	               <input id="search-field" type="text" autocomplete="off" data-control-parameter-name="search-query" value="{{ $searchQuery }}" class="form-control font-7 control-field" placeholder="საძიებო სიტყვა">
	 					...
	 					<select class="control-data-list col-sm-2 p-n mr20" data-control-parameter-name="{{ $manufacturersKey }}">
	 						<option class="font-6" value="0">ყველა ბრენდი</option>
	 						 @foreach($manufacturers as $manufacturer)
	 							 <option value="{{ $manufacturer -> id }}" {{ $selectedManufacturerId == $manufacturer -> id ? "selected" : null }} class="font-6"> {{ $manufacturer -> manufacturerTitle }} </option>
	 						 @endforeach
	 					</select>
	           ...
	           <ul class="pagination mt0 mb20 col-sm-5">
				 ...
				 <div class="tab-pane" id="add-product">
					 ...
					 <form action="{{ route('mbStore') }}" method="POST" class="form-horizontal row-border store-form" enctype="multipart/form-data">
  			       ...
  			       <input type="text" id="touchspin1" name="price" class="form-control font-7">
 						...
 						<div class="form-group">
							...
							<div class="col-sm-2">
 								 <select name="socketId" class="form-select wpr-100">
							...
							<div class="col-sm-2">
 								 <select name="chipsetId" class="form-select wpr-100">
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								<input type="text" class="form-control font-7" data-reg-exp="^([1-9]\d*)$" name="maxMemory">
							</div>

							<label class="col-sm-1 control-label font-6">სლოტები</label>
 							<div class="col-sm-2">
 								<input type="text" class="form-control font-7" data-reg-exp="^([1-9]\d*)$" name="ramSlots" value="2">
 							</div>

							<label class="col-sm-1 control-label font-6">ბრენდი</label>
							<div class="col-sm-2">
 								 <select name="manufacturerId" class="form-select wpr-100">
							...
							<label class="col-sm-1 control-label font-6">RAM ტიპი</label>
							<div class="col-sm-2">
 								 <select name="memoryTypeId" class="form-select wpr-100">
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								 <select id="solid-state-drives-select" multiple class="form-select wpr-100 populate">
									 @foreach($ssdTypes as $value)
										 <option class="font-6" value="{{ $value -> id }}">{{ $value -> formFactorTitle }}</option>
									 @endforeach
								 </select>
								 <input type="hidden" name="solidStateDriveTypeId" id="solid-state-drives">
							</div>
						</div>
...
$("#solid-state-drives-select").on("change", function(e) {

    let data = $(this).select2("data");
    let solidStateDrivesInput = document.querySelector("#solid-state-drives");
    let values = [];

    for(object of data) values.push(object["id"]);

    solidStateDrivesInput.value = values.join(",");
});
