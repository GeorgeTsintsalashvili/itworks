 ...
 <div class="panel-body">
  ...
  <div class="tab-content">
    ...
    <div class="tab-pane active" id="editor">
      ...
      <form class="form-horizontal record-update-form" method="POST" action="{{ route('ssdUpdate') }}">
          ...
          <div class="form-group">
           ...
           <div class="form-group">
            ...
            <label class="col-sm-1 control-label font-6">ტექნ-გია</label>
            <div class="col-sm-2">

             <select name="technologyId" class="edit-page-list wpr-100">
              @foreach($technologies as $value)
                @if($value -> id == $product -> technologyId)
                  <option selected class="font-6" value="{{ $value -> id }}">{{ $value -> technologyTitle }}</option>
                @else
                  <option class="font-6" value="{{ $value -> id }}">{{ $value -> technologyTitle }}</option>
                @endif
              @endforeach
            </select>
           </div>
  ...
  <div class="tab-pane" id="images">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="tab-content">
          <div class="row mb50">
            <div class="col-sm-4">
              <a href="/solidStateDrives/{{ $product -> id }}" target="_blank">
               <img id="main-image" src="/images/solidStateDrives/main/original/{{ $product -> mainImage }}" class="img-responsive img-thumbnail wpr-100">
            ...
            <div class="col-sm-8">
              <form method="POST" action="{{ route('ssdImageUpdate') }}" class="dropzone" id="main-image-dropzone" enctype="multipart/form-data">
                <div class="fallback">
    ...
    <div class="panel panel-default">
      ...
      <div class="tab-content">
         ...
         <div class="col-md-12">
             <div class="owl-carousel owl-theme">
               @foreach($images as $image)
                <div class="item">
                  <a href="/images/solidStateDrives/slides/original/{{ $image -> image }}" data-fancybox-group="button" class="fancy fancybox-buttons zoom-button">
                    <i class="fas fa-expand"></i>
                  </a>
                  <a href="{{ route('ssdImgDestroy', ['id' => $image -> id]) }}" class="image-delete-button">
                    <i class="fas fa-trash-alt"></i>
                  </a>
                  <img src="/images/solidStateDrives/slides/preview/{{ $image -> image }}">
                </div>
               @endforeach
             </div>
         </div>
        ...
        <div class="row mt40">
          <div class="col-md-12">
            <form method="POST" action="{{ route('ssdImageUpload') }}" class="dropzone" enctype="multipart/form-data" id="images-dropzone">
            </form>
          </div>
        </div>
