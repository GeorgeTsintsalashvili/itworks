	...
	<div class="col-xs-8">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="font-6">ლეპტოპის სისტემები</h4>
			</div>
			<div class="panel-body">
				<table class="table table-hover mb1" data-update-route="{{ route('laptopSystemUpdate') }}">
					<thead>
						<tr class="font-6">
							<th>სისტემა</th>
							<th>წაშლა</th>
          ...
          @foreach($laptopSystems as $value)
              ...
              <!--- Title field --->
							<td>
								<input data-parameter-name="laptop-system-title" type="text" value="{{ $value -> laptopSystemTitle }}" class="no-border transparent-bg-color font-7 field">
							</td>

              <!--- Delete button --->
							<td>
								<a class="delete-record" href="{{ route('laptopSystemDestroy', ['id' => $value -> id]) }}">
       ...
       <form action="{{ route('laptopSystemStore') }}" method="post" class="form-horizontal row-border data-form" enctype="multipart/form-data">
 				<div class="form-group">
          <div class="col-sm-3">
            <input type="submit" class="btn-primary btn font-6" value="დამატება">
          </div>
 					<div class="col-sm-4">
 						<input name="laptop-system-title" type="text" class="form-control font-7" placeholder="დასახელება">
 					</div>
        </div>
      </form>
