				...
				<table class="table table-hover mb1" data-update-route="{{ route('mmtypeUpdate') }}">
					...
					@foreach($ramTypes as $value)
						...
						<tr data-record-id="{{ $value -> id }}">
							...
							<!--- Title field --->
							<td>
								<input data-parameter-name="ram-type-title" type="text" value="{{ $value -> typeTitle }}" class="no-border transparent-bg-color font-7 field">
							</td>
            ...
            @endif
			 ...
			 <form action="{{ route('mmtypeStore') }}" method="post" class="form-horizontal row-border data-form" enctype="multipart/form-data">
				<div class="form-group">
					...
					<div class="col-sm-6">
						<input name="ram-type-title" type="text" class="form-control font-7" placeholder="დასახელება">
					</div>
				</div>
			</form>
