 ...
 <div class="panel-body">
  ...
  <div class="tab-content">
    ...
    <div class="tab-pane active" id="editor">
      ...
      <form class="form-horizontal record-update-form" method="POST" action="{{ route('mmUpdate') }}">
          ...
          <div class="form-group">
             ...
             <div class="col-sm-2">
                 <input type="text" name="unitsInGroup" class="form-control font-7" value="{{ $product -> unitsInGroup }}">
             </div>
           ...
           <div class="form-group">
             ...
             <div class="col-sm-2">
               <select name="destination" class="form-select wpr-100">
                 @foreach($destinations as $key => $destination)
                    <option {{ $product -> destination == $key ? "selected" : null }} class="font-6" value="{{ $key }}">{{ $destination }}</option>
                 @endforeach
               </select>
            </div>
             ...
             <div class="col-sm-2">
               <select name="memoryModuleTypeId" class="form-select wpr-100">
                 @foreach($types as $value)
                   @if($value -> id == $product -> memoryModuleTypeId)
            ...
            <div class="col-sm-2">
               <input type="text" name="frequency" class="form-control font-7" value="{{ $product -> frequency }}">
            </div>

            <label class="col-sm-1 control-label font-6">მოცულობა</label>
            <div class="col-sm-2">
               <input type="text" name="capacity" class="form-control font-7" value="{{ $product -> capacity }}">
            </div>
  ...
  <div class="tab-pane" id="images">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="tab-content">
          <div class="row mb50">
            <div class="col-sm-4">
              <a href="/memoryModules/{{ $product -> id }}" target="_blank">
               <img id="main-image" src="/images/memoryModules/main/original/{{ $product -> mainImage }}" class="img-responsive img-thumbnail wpr-100">
            ...
            <div class="col-sm-8">
              <form method="POST" action="{{ route('mmImageUpdate') }}" class="dropzone" id="main-image-dropzone" enctype="multipart/form-data">
                <div class="fallback">
    ...
    <div class="panel panel-default">
      ...
      <div class="tab-content">
         ...
         <div class="col-md-12">
             <div class="owl-carousel owl-theme">
               @foreach($images as $image)
                <div class="item">
                  <a href="/images/memoryModules/slides/original/{{ $image -> image }}" data-fancybox-group="button" class="fancy fancybox-buttons zoom-button">
                    <i class="fas fa-expand"></i>
                  </a>
                  <a href="{{ route('mmImgDestroy', ['id' => $image -> id]) }}" class="image-delete-button">
                    <i class="fas fa-trash-alt"></i>
                  </a>
                  <img src="/images/memoryModules/slides/preview/{{ $image -> image }}">
                </div>
               @endforeach
             </div>
         </div>
        ...
        <div class="row mt40">
          <div class="col-md-12">
            <form method="POST" action="{{ route('mmImageUpload') }}" class="dropzone" enctype="multipart/form-data" id="images-dropzone">
            </form>
          </div>
        </div>
